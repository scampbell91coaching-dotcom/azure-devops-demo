...
[[ $# -gt 0 ]] || die "Usage: $0 PAGE_NAME [--repo PATH]"
NAME="$1"; shift
while [[ $# -gt 0 ]]; do
  case "$1" in
    --repo) REPO="$2"; PORTAL="$REPO/platform-portal"; shift 2 ;;
    --help|-h) echo "Usage: $0 PAGE_NAME [--repo PATH]"; exit 0 ;;
...
[[ ! -e "$TEMPLATE" && ! -e "$JS" ]] || die "Page already exists"
mkdir -p "$JS_DIR"
...
{% block subtitle %}${CLASS} platform view{% endblock %}
{% block content %}
<section class="section">
  <div class="card">
    <p id="${KEBAB}-message">Page ready.</p>
  </div>
</section>
...
cat > "$JS" <<JS
document.getElementById("${KEBAB}-message").textContent = "${CLASS} loaded";
JS

ok "Page scaffold created"
echo "Add a route in portal/views.py and navigation in templates/base.html."
