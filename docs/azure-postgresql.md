...
The password is sensitive Terraform state data because Azure requires it during server creation. Remote state access must remain tightly restricted. Terraform never outputs the password. The non-secret `postgresql_connection` output exposes the hostname, port, database, TLS mode, and Key Vault secret names needed to configure AKS.
...
The `Azure PostgreSQL plan` GitHub Actions workflow validates pull requests. A manual dispatch accepts only the non-secret development server and Key Vault names, authenticates using the existing OIDC secrets, and uploads a one-day plan artifact. It contains no apply job. The older general Terraform workflow can apply on `main`; keep `postgresql_enabled = false` in its tracked tfvars until that workflow is separately changed to an approved plan/apply gate.

## Connecting from AKS

The module writes these Key Vault secrets, matching the existing External Secrets pattern:

| Key Vault secret | Purpose |
| --- | --- |
| `postgresql-host` | Private Flexible Server FQDN |
| `postgresql-port` | Port `5432` |
| `postgresql-database` | Application database |
| `postgresql-username` | Administrator login |
...
Deletion is destructive and is never automated by the PostgreSQL workflow. Before removal:
