...
| Files/PDFs | Migration `0024_pdf_meal_plan_delivery` stores immutable PDF bytes, length, and SHA-256 metadata in PostgreSQL; there is no separate application object store. | The database restore includes current PDF bytes. Drills must validate byte length and independently recompute every hash in Python; a future object store needs a separate recovery procedure. |
...
4. Run the verifier against the disposable restore:
   ...
   The script rejects equal, production-like, or ambiguously named targets; starts a read-only transaction; applies query/lock timeouts; and checks the Alembic head, counts, constraints, indexes, tenancy, sequences, and every restored PDF's byte length and SHA-256 without requiring `pgcrypto`. It emits no URI, hostname, row content, filename, athlete data, PDF bytes, or record-linked hash; PDF evidence is aggregate only. Exit 0 is necessary but not sufficient. See `docs/operations/restore-drill-runbook.md` for the complete procedure and cleanup checklist.
